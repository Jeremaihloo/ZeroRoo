...
{
    public class LaughingParser
    {
        LaughingLexer lexer;
        Token current;
        List<Element> elements;

        public LaughingParser(LaughingLexer lexer)
        {
            this.lexer = lexer;
        ...
        Token Consume(TokenKind kind)
        {
            ...
            if (old.TokenKind != kind)
                throw new ParseException(string.Format("Unexpected token: {0} Was expecting : {1} Line : {2} Col : {3}", 
                    current.TokenKind.ToString(), kind, current.Line, current.Col), 
